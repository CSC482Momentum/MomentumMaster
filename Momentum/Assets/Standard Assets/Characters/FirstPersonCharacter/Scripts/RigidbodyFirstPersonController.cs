...
public class RigidbodyFirstPersonController : NetworkBehaviour
{
    ...
    [Command]
    public void CmdAddForce2(Vector3 amt, String player)
    {
        RigidbodyFirstPersonController play = GameObject.FindGameObjectWithTag(player).GetComponent<RigidbodyFirstPersonController>();
    ...
    [ClientRpc]
    public void RpcAddServerForce(Vector3 amt)
    {
        if (isLocalPlayer)
        {
    ...
    public void ApplyForceToPlayer(Vector3 amt, String tag)
    {
        CmdAddForce2(amt, tag);
    }
    ...
    public class MovementSettings
    {
        ...
        public float maxSpeedAir = 20f;
        public float maxSpeedGround = 15f;
        [HideInInspector]
        public float CurrentTargetSpeed = 8f;
        [HideInInspector]
        public Boolean movingWithWeapon = false;
    ...
    private void FixedUpdate()
    {
        ...
        {
            GroundCheck();
            //gameObject.GetComponentInChildren<Assets.Weapon>();
            Vector2 input = GetInput();
            //print("Position: " + m_RigidBody.position);
            if ((Mathf.Abs(input.x) > float.Epsilon || Mathf.Abs(input.y) > float.Epsilon) && (advancedSettings.airControl || m_IsGrounded))
            {
                    ...
                    (movementSettings.CurrentTargetSpeed * movementSettings.CurrentTargetSpeed))
                {
                    ...
                    //RpcAddServerForce((desiredMove * SlopeMultiplier()));
                    //print("We're using the FPS controller to move!");
                    //RpcAddServerForce(desiredMove * SlopeMultiplier());
                }
            }
            ...
            if (m_IsGrounded)
            {
                ...
                if (m_Jump)
                {
                    ...
                    CmdAddForce(new Vector3(m_RigidBody.velocity.x, movementSettings.JumpForce, m_RigidBody.velocity.x), ForceMode.Impulse);
                    //RpcAddServerForce(new Vector3(m_RigidBody.velocity.x, movementSettings.JumpForce, m_RigidBody.velocity.x));
                    //RpcAddServerForce(new Vector3(0f, movementSettings.JumpForce, 0f));
                    m_Jumping = true;
                }
    ...
    public void testFire()
    {
        ...
        Vector3 yvectaerial = new Vector3(0, (float).8, 0);
        //resetSecondaryFire();
        if (CrossPlatformInputManager.GetButtonDown("Fire2") || Input.GetAxisRaw("Xbox Left Trigger") != 0)
        {
            if (isLocalPlayer)
            {
                ...
                Vector3 fwd = cam.transform.forward; //transform.parent.transform.TransformDirection(Vector3.forward); //here we're getting the direction of the camera.
                ...
                if (Physics.Raycast(transform.position, fwd, out hit, 30))
                {
                    if (hit.collider.tag == "Hook")
                    {
                        ...
                        if (Grounded)
                        {
                            m_RigidBody.drag = 5f;
                            m_Jump = true;
                            var temp = (((hit.point - transform.position).normalized) + yvectgrounded) * 800;
                            ...
                            CmdAddForce(temp, ForceMode.Impulse);
                        ...
                        else
                        {
                            ...
                            var temp = (((hit.point - transform.position).normalized) + yvectgrounded) * 1000;
                            //float step = Time.deltaTime;
                            //var temp = Vector3.MoveTowards(transform.position, hit.point, step);
                            CmdAddForce(temp, ForceMode.Impulse);
                            //transform.root.GetComponent<RigidbodyFirstPersonController>().CmdAddForce(temp, ForceMode.Impulse);
